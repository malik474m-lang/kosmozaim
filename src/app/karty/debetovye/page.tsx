...
export const metadata: Metadata = {
  title: "Дебетовые карты — Подбор дебетовых карт с кэшбеком | Космозайм",
  description:
    "Сравните дебетовые карты от ведущих банков. Кэшбек, бесплатное обслуживание и выгодные условия.",
  keywords: "дебетовые карты, дебетовая карта с кэшбеком, карта с бесплатным обслуживанием",
};
...
async function OffersList({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const conditions: SQL[] = [
    ...
    eq(offers.category, "debit_cards"),
  ];
...
export default async function DebitCardsPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-900 mb-2">Дебетовые карты</h1>
        <p className="text-gray-600">
          Подберите дебетовую карту с кэшбеком и бесплатным обслуживанием.
        </p>
      </div>

      <Suspense fallback={<div>Загрузка...</div>}>
        <OfferFilter category="debit_cards" />
      </Suspense>
      ...
      <div className="mt-12 bg-white rounded-xl shadow-sm border border-gray-100 p-8">
        <h2 className="text-xl font-bold text-gray-900 mb-4">Зачем нужна дебетовая карта?</h2>
        <div className="prose prose-sm text-gray-600 max-w-none">
          <p>
            Дебетовые карты позволяют удобно оплачивать покупки и получать кэшбек. Многие банки
            предлагают бесплатное обслуживание и начисление процентов на остаток средств.
          </p>
        </div>
      </div>
    </div>
