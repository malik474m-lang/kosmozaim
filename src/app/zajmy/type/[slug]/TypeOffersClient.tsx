...
import { formatDays, formatMoney, normalizeMediaUrl } from "@/lib/utils";

interface TypeOffersClientProps {
  filterParams?: {
    freeTermDays?: number;
...
export default function TypeOffersClient({ filterParams }: TypeOffersClientProps) {
  const { geo } = useGeo();
  ...
  useEffect(() => {
    const fetchOffers = async () => {
      setLoading(true);
      try {
        const params = new URLSearchParams({ category: "microloans" });
  ...
  const filteredOffers = useMemo(() => {
    return offers.filter((offer) => {
      if (filterParams?.freeTermDays && offer.freeTermDays < filterParams.freeTermDays) {
        return false;
      }

      if (filterParams?.borrowerCategory) {
        const borrowerMatch =
          offer.borrowerCategory === filterParams.borrowerCategory ||
          offer.borrowerCategory === "any";
        if (!borrowerMatch) return false;
      }

      if (filterParams?.amountMin && offer.amountMax < filterParams.amountMin) {
        return false;
      }

      if (filterParams?.amountMax && offer.amountMin > filterParams.amountMax) {
        return false;
      }
