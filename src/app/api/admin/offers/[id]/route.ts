...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    await db.update(offers).set({ title: body.title, category: body.category, amountMin: parseInt(body.amountMin) || 1000, amountMax: parseInt(body.amountMax) || 100000, termMinDays: parseInt(body.termMinDays) || 1, termMaxDays: parseInt(body.termMaxDays) || 365, psk: body.psk || "0", rate: body.rate || "0", freeTermDays: parseInt(body.freeTermDays) || 0, logoUrl: body.logoUrl || "", affiliateUrl: body.affiliateUrl, borrowerCategory: body.borrowerCategory || "any", description: body.description || "", seoKeywords: body.seoKeywords || "", regions: body.regions || "", isActive: body.isActive, sortOrder: parseInt(body.sortOrder) || 0, updatedAt: new Date() }).where(eq(offers.id, parseInt(id)));
    const updated = await db.select().from(offers).where(eq(offers.id, parseInt(id))).limit(1);
